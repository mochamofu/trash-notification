...
- `@Environment(\.colorScheme)`で現在の外観を参照
- カラーは直接指定せず、アプリ用のsemantic colorに寄せる
- 今日カード、通常カード、広告枠、タブ背景をそれぞれライト/ダークで定義
- ダークモードは黒一色にせず、深い緑系の背景とカードで生活アプリらしい柔らかさを保つ
- テーマ強制切り替えを入れる場合は`AppStorage`で保存する
